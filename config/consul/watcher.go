...
func newWatcher(key, addr, token, name, stripPrefix string,
	isConfigFile bool, e encoder.Encoder) (source.Watcher, error) {
	w := &watcher{
		e:            e,
		name:         name,
		key:          key,
...
func (w *watcher) handle(idx uint64, data interface{}) {
	...
	d, err := makeMap(w.e, kvs, w.key, w.isConfigFile)
	if err != nil {
		return
	}
